...
namespace VRA.BusinessLayer
{
    public class ReportGenerator
    {
        ...
        public string fillHtmlTableByType(List<object> grid, string html, string status)
        {
            string table = "";

            String col_type = "auto"; // or "manual"
            ...
            switch (col_type)
            {
                case "auto":
                    ...
                    // пустой объект-контейнер текущей записи
                    ...
                    foreach (var item in grid)
                    {
                        ...
                        foreach (var prop in itemObj.GetType().GetProperties())
                        {
                            object t = prop.GetValue(itemObj, null);
                            // здесь начинается
                            if (t != null && t.GetType() == typeof(NationDto))
                            {
                                // Костыль, который подпирает национальность художника
                                NationDto temp = (NationDto)t;
                                // Костыль поставил @MrAnderson
                                val = temp.Nationality.ToString();
                            }
                            else
                            {
                                val = (t == null) ? "" : t.ToString();
            ...
            return html.Replace("[VRA_TABLE_REPORT]", table);
        }
